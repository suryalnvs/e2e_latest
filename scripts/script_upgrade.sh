...
installChaincode () {
	...
	do
		...
		do
			PEER=$i
			setGlobals $PEER
			peer chaincode install -n mycc$var$ch -v $version -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02 >&log.txt
			res=$?
			cat log.txt
		        verifyResult $res "Chaincode 'mycc$var$ch' installation on remote peer PEER$PEER has Failed"
			echo "===================== Chaincode 'mycc$var$ch' is installed on remote peer PEER$PEER ===================== "
			echo
		done
	done
}
...
instantiateChaincode () {
	...
	do
		...
		do
			#PEER=` expr $ch \/ 4`
			#setGlobals $PEER
			peer chaincode instantiate -o $ORDERER_IP:7050 -C $CHANNEL_NAME$i -n mycc$var$ch -v a -c '{"Args":["init","a","1000","b","2000"]}' -P "OR	('Org0MSP.member','Org1MSP.member')" >&log.txt
			res=$?
			cat log.txt
			#verifyResult $res "Chaincode 'mycc$ch' instantiation on PEER$PEER on channel '$CHANNEL_NAME$i' failed"
			echo "===================== Chaincode 'mycc$var$ch' Instantiation on PEER$PEER on channel '$CHANNEL_NAME$i' is successful ===================== "
			echo
		done
	done
}
...
installUpgradedChaincode () {
	...
	do
		...
		do
			PEER=$i
			setGlobals $PEER
			peer chaincode install -n mycc$var$ch -v $version -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02_noquery >&log.txt
			res=$?
			cat log.txt
		        verifyResult $res "Chaincode 'mycc$var$ch' upgrade install on remote peer PEER$PEER has Failed"
			echo "===================== Chaincode 'mycc$var$ch' upgrade installed on remote peer PEER$PEER ===================== "
			echo
		done
	done
}
...
upgradeOldChaincode () {
	...
	do
		...
		do
			#PEER=` expr $ch \/ 4`
			#setGlobals $PEER
			peer chaincode upgrade -o $ORDERER_IP:7050 -C $CHANNEL_NAME$i -n mycc$var$ch -v $version -c '{"Args":["init","a","1000","b","2000"]}' -P "OR	('Org0MSP.member','Org1MSP.member')" >&log.txt
			res=$?
			cat log.txt
			verifyResult $res "Chaincode 'mycc$var$ch' upgrade  on PEER$PEER on channel '$CHANNEL_NAME$i' failed"
			echo "===================== Chaincode 'mycc$var$ch' upgrade on PEER$PEER on channel '$CHANNEL_NAME$i' is successful ===================== "
			echo
		done
	done
}
...
chaincodeInvoke () {
	...
	peer chaincode invoke -o $ORDERER_IP:7050 -C $CHANNEL_NAME$channel_num -v $version -n mycc$var$chain_num -c '{"Args":["invoke","a","b","10"]}' >&log.txt
	res=$?
	cat log.txt
...
chaincodeQuery () {
  ...
  local var=$5
  local version=$6
  echo "===================== Querying on PEER$peer on $CHANNEL_NAME$channel_num/mycc$var$chain_num... ===================== "
  local rc=1
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME$channel_num -v $version -n mycc$var$chain_num -c '{"Args":["query","a"]}' >&log.txt
...
chaincodeQueryUpgrade () {
  ...
  do
     sleep 3
     echo "Attempting to Query PEER$peer ...$(($(date +%s)-starttime)) secs"
     peer chaincode query -C $CHANNEL_NAME$channel_num -v b -n mycc1$chain_num -c '{"Args":["query","a"]}' >&log.txt
     test $? -eq 0 && VALUE=$(cat log.txt | awk '/Query Result/ {print $NF}')
     test "$VALUE" = "$res" && let rc=0
  done
  echo
  ...
  if test $rc -eq 0 ; then
        ...
        echo "!!!!!!!!!!!!!!! Query result on PEER$peer is INVALID !!!!!!!!!!!!!!!!"
        installChaincode $version $var
        upgradeOldChaincode 0 $version $var
        ...
        do
            ...
            do
                ...
                do
                        ...
                        chaincodeQuery $ch $chain $peer_number "$AVAL" 1 a
                        chaincodeInvoke $ch $chain $peer_number 1 a
                        AVAL=` expr $AVAL - 10 `
                        chaincodeQuery $ch $chain $peer_number "$AVAL" 1 a
                done
            done
...
installUpgradedChaincode b 1
echo " ************** Calling upgrade old chaincode ************"
upgradeOldChaincode 0 b 1

# invoke channel_num, chaincode_num, peer, expected value
chaincodeQueryUpgrade 0 0 0 1000 c 1
# invoke channel_num, chaincode_num, peer
chaincodeInvoke 0 0 0 1 a
# invoke channel_num, chaincode_num, peer, expected value
chaincodeQuery 0 0 0 10000 1 1.0
