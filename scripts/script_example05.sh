...
installChaincode () {
	...
	do
		...
		do
			PEER=$i
			setGlobals $PEER
			peer chaincode install -n mycc05$ch -v 1 -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example05 >&log.txt
			peer chaincode install -n mycc02$ch -v 1 -p github.com/hyperledger/fabric/examples/chaincode/go/chaincode_example02 >&log.txt
			res=$?
			cat log.txt
...
instantiateChaincode () {
	...
	do
		...
		do
			#PEER=` expr $ch \/ 4`
			#setGlobals $PEER
			peer chaincode instantiate -o $ORDERER_IP:7050 -C $CHANNEL_NAME$i -n mycc05$ch -v 1 -c '{"Args":["init", "sum", "0"]}' -P "OR('Org0MSP.member','Org1MSP.member')" >&log.txt
			res=$?
			cat log.txt
			verifyResult $res "Chaincode 'mycc$ch' instantiation on PEER$PEER on channel '$CHANNEL_NAME$i' failed"
			echo "===================== Chaincode 'mycc05$ch' Instantiation on PEER$PEER on channel '$CHANNEL_NAME$i' is successful ===================== "
			echo

peer chaincode instantiate -o $ORDERER_IP:7050 -C $CHANNEL_NAME$i -n mycc02$ch -v 1 -c '{"Args":["init", "a", "100", "b", "200"]}' -P "OR('Org0MSP.member','Org1MSP.member')" >&log.txt
			res=$?
			cat log.txt
			verifyResult $res "Chaincode 'mycc$ch' instantiation on PEER$PEER on channel '$CHANNEL_NAME$i' failed"
			echo "===================== Chaincode 'mycc02$ch' Instantiation on PEER$PEER on channel '$CHANNEL_NAME$i' is successful ===================== "
			echo
		done
	done
}
...
chaincodeInvokeEx02() {
	...
	local chain_num=$2
        local peer=$3
	peer chaincode invoke -o $ORDERER_IP:7050  -C $CHANNEL_NAME$channel_num -n mycc02$chain_num -c '{"Args":["invoke","a", "b", "10"]}' >&log.txt
	res=$?
	cat log.txt
	verifyResult $res "Invoke execution on PEER$peer failed "
	echo "===================== Invoke transaction on PEER$peer on $CHANNEL_NAME$channel_num/mycc02$chain_num is successful ===================== "
	echo
}
...
chaincodeInvokeEx05() {
	...
	local chain_num=$2
        local peer=$3
	peer chaincode invoke -o $ORDERER_IP:7050  -C $CHANNEL_NAME$channel_num -n mycc05$chain_num -c '{"Args":["invoke","mycc020", "sum"]}' >&log.txt
	res=$?
	cat log.txt
	verifyResult $res "Invoke execution on PEER$peer failed "
	echo "===================== Invoke transaction on PEER$peer on $CHANNEL_NAME$channel_num/mycc05$chain_num is successful ===================== "
	echo
}
...
chaincodeQuery () {
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME$channel_num -n mycc05$chain_num -c '{"Args":["query","mycc020", "sum"]}' >&log.txt
...
do
	...
	do
		...
		chaincodeQuery $ch $chain $PEER 300 
		chaincodeInvokeEx02 $ch $chain $PEER
                sleep 20
		chaincodeQuery $ch $chain $PEER 300
	done
done
