...
assert () {
 eval "${1}"
 if [[ $? -ne 0 ]]; then
   echo "${FUNCNAME[1]}: failed" >& cli.txt
 else
   echo "${FUNCNAME[1]}: passed" >& cli.txt
 fi
}
...
do
	...
	do
		...
		do
			...
			assert "chaincodeQuery $ch $chain $peer_number '$AVAL'"
			assert "chaincodeInvoke $ch $chain $peer_number"
			AVAL=` expr $AVAL - 10 `
			assert "chaincodeQuery $ch $chain $peer_number "$AVAL""
		done
	done
